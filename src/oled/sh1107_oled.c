...
#define OLED_SPI_CS_PIN 5
#define OLED_SPI_SCK_PIN 6
#define OLED_SPI_TX_PIN 7

#define OLED_CMD_DATA_SWT    15
#define ssh1107_oled_spi spi0

#else

#define OLED_SPI_RX_PIN 8
#define OLED_SPI_TX_PIN 11

#define OLED_SPI_CS_PIN  9
#define OLED_SPI_SCK_PIN 10

#define OLED_CMD_DATA_SWT    20
#define ssh1107_oled_spi spi1

#endif

void init_oled_screen() {
    // This example will use SPI0 at 0.5MHz.
    spi_init(ssh1107_oled_spi, 1 * 1000 * 1000);
    ...
    gpio_init(OLED_CMD_DATA_SWT);
    gpio_set_dir(OLED_CMD_DATA_SWT, GPIO_OUT);
}
...
#ifdef i2c_default
//数据/命令信号选择，低电平表示命令，高电平表示数据
void SSD1306_send_cmd(uint8_t cmd) {
    // I2C write process expects a control byte followed by data
    // this "data" can be a command or data to follow up a command
    // Co = 1, D/C = 0 => the driver expects a command
    uint8_t buf[2] = {0x80, cmd};
    gpio_put(OLED_CMD_DATA_SWT, 0);
    spi_write_blocking(ssh1107_oled_spi, buf, 2);
}
...
//    i2c_write_blocking(i2c_default, (SSD1306_I2C_ADDR & SSD1306_WRITE_MODE), temp_buf, buflen + 1, false);
    gpio_put(OLED_CMD_DATA_SWT, 1);
    spi_write_blocking(ssh1107_oled_spi, temp_buf, buflen + 1);
    free(temp_buf);
}

void SSD1306_init() {
    // Some of these cmds are not strictly necessary as the reset
