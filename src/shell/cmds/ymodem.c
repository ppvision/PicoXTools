...
extern void km_tty_putc(char ch);
...
uint32_t km_tty_read_sync(uint8_t *buf, size_t len, uint32_t timeout) {
  ...
  do {
    int c = getchar_timeout_us(1*1000);
    if (c != PICO_ERROR_TIMEOUT) {
        buf[rd_len++] = (uint8_t)c;
    }
  } while (absolute_time_diff_us(get_absolute_time(),timeout_ms)>=0 && sz < len);
...
static int header_cb(uint8_t *file_name, size_t file_size) {
    if (check_mount(true))
        return -1;
    if (!file_name){
        sprintf(result, "file_name is empty");
        return -1;
    }
    int fs_err = fs_mkdir("/upload");
    if (fs_err < LFS_ERR_OK && fs_err != -17) {//exist
        sprintf(result,"Can't create Dir:/upload");
        return -1;
    }
    sprintf(path,"/upload/%s",file_name);
    if (fs_file_open(&ymodem_file, path, LFS_O_WRONLY | LFS_O_CREAT | LFS_O_TRUNC) < LFS_ERR_OK) {
        sprintf(result,"Can't create file");
        return -1;
    }
...
static int packet_cb(uint8_t *data, size_t len) {
    ...
    if (fs_file_write(&ymodem_file, data, len) != len){
...
static void footer_cb() {
    printf("\nfile transfered, size: %d", fs_file_seek(&ymodem_file, 0, LFS_SEEK_END));
    fs_file_close(&ymodem_file);
    bytes_remained = 0;
}

km_ymodem_status_t ymodem_rx(){
    return km_ymodem_receive( header_cb, packet_cb, footer_cb);
}
