...
static void check_context(char cmd) {
    // Certain movement cmds update the context.
...
static void undo_push(char* src, uint32_t length, int u_type) {
    ...
    // performs with a single action, i.e. REPLACE mode or find-and-replace cmds
...
#define GOT_RANGE ((got & 3) == 3)
    ...
    *cmdend = '\0';
    ...
    // assume the command will want a range, certain cmds
...
static int find_range(char** start, char** stop, int cmd) {
    ...
    if ((cmd == 'Y' || cmd == c) && strchr("cdy><", c)) {
        ...
        // don't '%' needs no special treatment, search cmds are
